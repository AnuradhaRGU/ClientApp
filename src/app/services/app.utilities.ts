...
import { HttpResponseBase, HttpHeaders, HttpResponse, HttpErrorResponse } from '@angular/common/http'
...
export class AppUtilities {
  ...
  public static findHttpResponseMessage(messageToFind: string, data: HttpResponse<any> | any, seachInCaptionOnly = true, includeCaptionInResult = false): string {
    ...
    for (let message of httpMessages) {
      let fullMessage = AppUtilities.splitInTwo(message, this.captionAndMessageSeparator);
  ...
  public static getHttpHeadersApplicationJson(token?: string) {
  ...
  public static getHttpHeadersMultipartApplicationJson(token?: string) {
    return new HttpHeaders({
       'Accept': 'application/json',
       'Access-Control-Allow-Headers':' Content-Type, Authorization',
       'authorization': (token != null && token != undefined) ? 'bearer ' + token : ''
  ...
  public static getQueryParamsFromString(paramString: string) {
    ...
    for (let param of paramString.split("&")) {
      let keyValue = AppUtilities.splitInTwo(param, "=");
  ...
  public static JSonstringify(value: any) {
    try {
  ...
  public static printDate(date: Date, separator = "at") {
    return `${AppUtilities.printDateOnly(date)} ${separator} ${AppUtilities.printTimeOnly(date)}`;
  }
  ...
  public static printFriendlyDate(date: Date, separator = "-") {
    ...
    if (test.toDateString() == today.toDateString())
      return `Today ${separator} ${AppUtilities.printTimeOnly(date)}`;
    if (test.toDateString() == yesterday.toDateString())
      return `Yesterday ${separator} ${AppUtilities.printTimeOnly(date)}`;
    else
      return AppUtilities.printDate(date, separator);
  }
  ...
  public static printShortDate(date: Date, separator = "/", dateTimeSeparator = "-") {
    ...
    return `${month}${separator}${day}${separator}${year} ${dateTimeSeparator} ${AppUtilities.printTimeOnly(date)}`;
  }

  public static printShortDateOnly(date: Date, separator = "/") {
  ...
  public static convertToAbsoluteUrl(url: string) {
    ...
    return AppUtilities.testIsAbsoluteUrl(url) ? url : '//' + url;
  }
  ...
  public static removeNulls(obj) {
    ...
    for (let k in obj) {
      if (obj[k] === null) {
        ...
        AppUtilities.removeNulls(obj[k]);
      }

      if (isArray && obj.length == k) {
        AppUtilities.removeNulls(obj);
      }
    }
